...
class CatalogSectionComponent extends ElementList
{
	...
	protected function initMetaData()
	{
		...
		if ($this->arParams['SET_TITLE'])
		{
			...
			{
				$this->arResult['IPROPERTY_VALUES']['SECTION_PAGE_TITLE'] = Regions::is_region() ? str_replace(" #region_case#", " â " . Regions::get_name_case_region(5), $this->arResult['IPROPERTY_VALUES']['SECTION_PAGE_TITLE']) : str_replace(" #region_case#", "", $this->arResult['IPROPERTY_VALUES']['SECTION_PAGE_TITLE']);
		...
		if ($this->arParams['SET_BROWSER_TITLE'] === 'Y')
		{
			...
			{				
				$APPLICATION->SetPageProperty('title', implode(' ', $browserTitle).(Regions::is_region() ? " â " . Regions::get_name_case_region(5) : ""), $this->storage['TITLE_OPTIONS']);
			}
			elseif ($browserTitle != '')
			{
				$browserTitle = Regions::is_region() ? str_replace(" #region_case#", " â " . Regions::get_name_case_region(5), $browserTitle) : str_replace(" #region_case#", "", $browserTitle);
		...
		if ($this->arParams['SET_META_KEYWORDS'] === 'Y')
		{
			...
			elseif ($metaKeywords != '')
			{
				$metaKeywords =Regions::is_region() ? str_replace(" #region_case#", " â " . Regions::get_name_case_region(5), $metaKeywords) : str_replace(" #region_case#", "", $metaKeywords);
				$APPLICATION->SetPageProperty('keywords', $metaKeywords.(Regions::is_region() ? " â " . Regions::get_name_case_region(5) : ""), $this->storage['TITLE_OPTIONS']);
			}
		}
		...
		if ($this->arParams['SET_META_DESCRIPTION'] === 'Y')
		{
			...
			{
				$metaDescription =Regions::is_region() ? str_replace(" #region_case#", " â " . Regions::get_name_case_region(5), $metaDescription) : str_replace(" #region_case#", "", $metaDescription);
		...
		{			
			foreach ($this->arResult['PATH'] as $path)
			{
				...
				{
					$path['IPROPERTY_VALUES']['SECTION_PAGE_TITLE'] = Regions::is_region() ? str_replace(" #region_case#", " â " . Regions::get_name_case_region(5), $path['IPROPERTY_VALUES']['SECTION_PAGE_TITLE']) : str_replace(" #region_case#", "", $path['IPROPERTY_VALUES']['SECTION_PAGE_TITLE']);
					$APPLICATION->AddChainItem($path['IPROPERTY_VALUES']['SECTION_PAGE_TITLE'], $path['~SECTION_PAGE_URL']);
				}
				else
				{
					$path['NAME'] = Regions::is_region() ? str_replace(" #region_case#", " â " . Regions::get_name_case_region(5), $path['NAME']) : str_replace(" #region_case#", "", $path['NAME']);
